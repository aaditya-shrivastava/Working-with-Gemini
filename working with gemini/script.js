document.getElementById('generateForm').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      // Filter out stars from the response text and split into bullet points
      const filteredText = data.text.replace(/\*/g, "").trim();
      const points = filteredText.split(/\n|\r\n/).filter(point => point.trim() !== "");
  
      // Create an unordered list for the points
      const responseDiv = document.createElement('div');
      responseDiv.classList.add('response');
      let pointsHtml = `<ul>`;
      points.forEach(point => {
        pointsHtml += `<li>${point.replace(/^[.\-•\s]+/, '')}</li>`;
      });
      pointsHtml += `</ul>`;
